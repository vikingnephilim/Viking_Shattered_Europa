...
PixelShader = 
{
	...
	[[
		float4 main( VS_OUTPUT_PDXMESHSTANDARD In ) : PDX_COLOR
		{
			...
			float4 vFoW = GetFoW( vPos, vFoWColor, FoWDiffuse );

			if( vFoWOpacity_Time.x < 0.0f ) {
				vFoW.a = 0.0;
			}

			vColor.rgb = ComposeSpecular( vColor.rgb, CalculateSpecular( vPos, vNormal, (vSpecColor.a * 2.0 ) ) * ( 1.0f - vFoW.a ) );
			vColor.rgb = CalculateLighting( vColor.rgb, vNormal );
			vColor.rgb = ApplyFoW( ApplyDistanceFog( vColor.rgb, vPos ), vFoW );
	...
	[[
		...
		float4 main( VS_OUTPUT_PDXMESHSTANDARD In ) : PDX_COLOR
		{
			...
			float4 vFoW = GetFoW( vPos, vFoWColor, FoWDiffuse );
			vColor.rgb = ComposeSpecular( vColor.rgb, CalculateSpecular( vPos, vNormal, (vSpecColor.a * 2.0 ) ) * vFoW.a );
			vColor.rgb = CalculateLighting( vColor.rgb, vNormal );
			vColor.rgb = ApplyFoW( ApplyDistanceFog( vColor.rgb, vPos ), vFoW );
			return vColor;
		}
